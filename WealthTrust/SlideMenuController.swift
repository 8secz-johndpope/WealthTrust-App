...
extension UIViewController {
    ...
    public func addBackArrowButtonWithImage(buttonImage: UIImage) {
        let leftButton: UIBarButtonItem = UIBarButtonItem(image: buttonImage, style: UIBarButtonItemStyle.Plain, target: self, action: #selector(self.toggelBack))
        navigationItem.leftBarButtonItem = leftButton;
    }

    
    public func addRightBarButtonWithImage(buttonImage1: UIImage, buttonImage2: UIImage) {
        
        let rightButton: UIBarButtonItem = UIBarButtonItem(image: buttonImage1, style: UIBarButtonItemStyle.Plain, target: self, action: #selector(self.opneProfile))
        rightButton.imageInsets = UIEdgeInsetsMake(0.0, -10, 0.0, 0.0);
        
        let rightButton2: UIBarButtonItem = UIBarButtonItem(image: buttonImage2, style: UIBarButtonItemStyle.Plain, target: self, action: #selector(self.opneSearch))
        rightButton2.imageInsets = UIEdgeInsetsMake(0.0, 0.0, 0, -15);
        
        navigationItem.rightBarButtonItems = [rightButton, rightButton2]
  
    }
    
    public func addRightBarButtonWithImageForTransaction(buttonImage2: UIImage) {
        
        let rightButton2: UIBarButtonItem = UIBarButtonItem(image: buttonImage2, style: UIBarButtonItemStyle.Plain, target: self, action: #selector(self.opneSearch))
        rightButton2.imageInsets = UIEdgeInsetsMake(0.0, -10, 0, 0);
        
        navigationItem.rightBarButtonItems = [rightButton2]
        
    }

    public func addRightBarButtonWithImageForPortfolio(buttonImage2: UIImage) {
    ...
    public func opneProfile() {
        ...
        if allUser.count==0
        {
            ...
            objUserLoginSignYp.isFrom = IS_FROM.Profile
            self.navigationController?.pushViewController(objUserLoginSignYp, animated: true)

        }else{
            let objUseRProfile = self.storyboard?.instantiateViewControllerWithIdentifier(ksbIdUserProfile) as! UserProfile
            self.navigationController?.pushViewController(objUseRProfile, animated: true)
        }
    }
    public func opneSearch() {
    ...
    public func openWebWith(viewC : UIViewController) {
        print("Open WEB")
