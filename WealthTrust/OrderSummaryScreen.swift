...
//  WealthTrust
//
//  Created by Hemen Gohil on 9/27/16.
...
class OrderSummaryScreen: UIViewController {
    ...
    @IBOutlet weak var lblSwitchTo: UILabel!
    ...
    override func viewDidLoad() {
        ...
        lblSwitchTo.text = "Switch to : \(objOrder.TarSchemeName)"
...
//            }
    ...
    @IBAction func btnContinueClicked(sender: AnyObject) {
        ...
        if objOrder.AppOrderTimeStamp==nil {
            objOrder.AppOrderTimeStamp = UpdateTime
        }
...
//            SharedManager.invokeAlertMethod("", strBody: "Error in saving record.", delegate: nil)
        ...
        var investmentAOFStatusCheck = "0"
        let allUser = DBManager.getInstance().getAllUser()
        if allUser.count==0
        {
        }else{
            ...
            if objUser.InvestmentAofStatus=="3" {
                investmentAOFStatusCheck = "0" // Switch without Doc
            }else{
                investmentAOFStatusCheck = "1"
            }
        }
