...
@property (nonatomic, retain) NSUserDefaults *objDefault;
...
+(void)savePasscodeDigit{
    
    NSUserDefaults *objDefault = [NSUserDefaults standardUserDefaults];
    
    [LTHKeychainUtils storeUsername:@"demoPasscode"
                        andPassword:[objDefault valueForKey:@"passcode"]
                     forServiceName:@"demoServiceName"
                     updateExisting:YES
                              error:nil];
}
...
- (void)_savePasscode:(NSString *)passcode {
    _objDefault = [NSUserDefaults standardUserDefaults];
    if (passcode != nil) {
        [_objDefault setValue:passcode forKey:@"passcode"];
    }
...
- (void)viewDidLoad {
    ...
    //
    //    UIBarButtonItem *buttonOne = [[UIBarButtonItem alloc] initWithTitle:@"Forgot Pin?" style:UIBarButtonItemStylePlain target:self action:@selector(action)];
    //
    //    NSArray *buttons = [NSArray arrayWithObjects: flexibleSpace, buttonOne,flexibleSpace2, nil];
...
- (void)_setupNavBarWithLogoutTitle:(NSString *)logoutTitle {
    ...
    if ([self respondsToSelector:@selector(setEdgesForExtendedLayout:)]) {
        self.navBar.barTintColor = [UIColor colorWithRed:27/250.f green:129/255.f blue:224/255.f alpha:1];
    ...
    [[UIBarButtonItem alloc] initWithTitle:@"Forgot? "
                                    ...
                                    target:self
                                    action:@selector(ForgotPinTap:)];
    
    UINavigationItem *item =
    [[UINavigationItem alloc] initWithTitle:@"WealthTrust"];
    item.rightBarButtonItem = leftButton;
    item.hidesBackButton = YES;
    
    //    UIBarButtonItem *flipButton = [[UIBarButtonItem alloc]initWithTitle:@"Forgot?" style:UIBarButtonItemStyleBordered target:self action:@selector(ForgotPinTap:)];
    //
...
- (void)_setupLabels {
    ...
    _btnForgotPassword = [[UIButton alloc]initWithFrame:CGRectZero];
    [_btnForgotPassword setTitle:@"Forgot Pin? " forState:UIControlStateNormal];
    _btnForgotPassword.titleLabel.textAlignment = NSTextAlignmentCenter;
    [_animatingView addSubview:_btnForgotPassword];
...
- (void)_setupDigitFields {
    ...
    int yPos = _failedAttemptLabel.frame.size.height+_failedAttemptLabel.frame.origin.y;
...
- (void)updateViewConstraints {
    ...
    CGFloat yOffsetFromCenter = -self.view.frame.size.height * 0.34 + _verticalOffset;
...
- (void)showLockscreenWithoutAnimation {
    [self showLockScreenWithAnimation:NO withLogout:NO andLogoutTitle:@"wealthTrust"];
}
...
- (void)_prepareNavigationControllerWithController:(UIViewController *)viewController {
    ...
    //    (red:27/250, green:129/250, blue:224/250, alpha: 1)
    if ([self respondsToSelector:@selector(setEdgesForExtendedLayout:)]) {
        navController.navigationBar.barTintColor = [UIColor colorWithRed:27/250.f green:129/255.f blue:224/255.f alpha:1];
...
-(void)ForgotPinTap:(id)sender
{
    if (_isFromBackground == true) {
        [self.navBar setHidden:YES];
    }
    //[self _dismissMe];
    UIStoryboard *strBoard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    LoginUser *obj = [strBoard instantiateViewControllerWithIdentifier:@"LoginUser"];
    obj.isFromPassCodeScreen = YES;
...
- (void)keyboardWasShown:(NSNotification *)notification
{
    ...
    //
    //    UIButton *btnForgot = [[UIButton alloc]initWithFrame:CGRectMake(15,self.view.frame.size.width-height , self.view.frame.size.width -30, 30)];
    //    [btnForgot setTitle:@"ForgotPassword" forState:UIControlStateNormal];
    //    [btnForgot setBackgroundColor:[UIColor redColor]];
    //    [_backgroundImageView addSubview:btnForgot];
